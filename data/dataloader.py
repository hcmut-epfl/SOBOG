...
class Twibot22SampleDataset(Dataset):
    ...
    def group_user_tweets_and_matrices(self, ids, tweet, edge, keep_indices, tfidf=None):
        ...
        for i, user_id in pbar:
            ...
            all_tweet_dfs = all_tweet_dfs.merge(self.label, how="left", left_on="author_id", right_on="id", suffixes=('', '_'))
            ...
            if keep_indices:
                ...
                tweet_adj = csr_matrix((np.ones((n_edges,)), (user_edges["source_index"], user_edges["target_index"])), shape=(n_tweets, n_tweets)).toarray()
    ...
    def save(self):
        filename = self.get_file_path(self.sample, self.train, self.use_tfidf, self.normalize)
        torch.save(self, filename)
    
    @staticmethod    
    def get_file_path(sample, train, use_tfidf, normalize):
        return 'data/twibot_22_{}_{}_{}_{}.pt'.format(
            "sample" if sample else "full",
            "train" if train else "test",
            "text-vectorized" if use_tfidf else "text-raw",
